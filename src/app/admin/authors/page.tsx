...
export default function AuthorsManagementPage() {
  ...
  const handleSave = async () => {
    ...
    const isGrantingAuthor = modalMode === 'add' || !selectedUser.is_author;

    const result = await updateUserRole(selectedUser.user_id, {
      name: formData.name,
      bio: formData.bio,
      avatar_url: formData.avatar_url,
      is_author: isGrantingAuthor ? true : selectedUser.is_author
    });
            ...
            {authors.map((author) => (
              <div key={author.user_id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-muted/30 transition-colors">
                ...
                <div className="flex gap-2">
                  ...
                  {!author.is_admin && author.is_author && (
                    <Button variant="destructive" size="sm" onClick={() => handleRemoveRole(author.user_id)}>
                ...
                <div className="space-y-4 animate-in slide-in-from-right-4 duration-200">
                  ...
                  <div className="space-y-2">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Αφήστε κενό για να χρησιμοποιηθεί η προεπιλεγμένη εικόνα.
                    </p>
                  </div>
