...
import { createCity, updateCity, getPrefectures } from "@/lib/api";
import { City, Prefecture, Attraction, LocalProduct } from "@/lib/types";
...
export function CityForm({ city }: CityFormProps) {
    ...
    // Tourist data states
    const [photoGallery, setPhotoGallery] = useState<string[]>(city?.photo_gallery || []);
    const [attractions, setAttractions] = useState<Attraction[]>(city?.attractions || []);
    const [howToGetThere, setHowToGetThere] = useState(city?.how_to_get_there || "");
    const [touristInfo, setTouristInfo] = useState(city?.tourist_info || "");
    const [events, setEvents] = useState<Event[]>(city?.events_festivals || []);
    const [localProducts, setLocalProducts] = useState<LocalProduct[]>(city?.local_products || []);
    ...
    const handleGetCoordinates = async () => {
        ...
        try {
            // Find selected prefecture to provide context
            const selectedPrefecture = prefectures.find(p => p.id === formData.prefecture_id);
            const parentName = selectedPrefecture?.name;
            ...
            const response = await fetch("/api/get-coordinates", {
                ...
                body: JSON.stringify({
                    ...
                    parentName: parentName // Send prefecture name for context
                }),
            });
                    ...
                    >
                        ...
                        {prefectures.map((prefecture) => (
                            <option key={prefecture.id} value={prefecture.id}>
                                {prefecture.name} ({prefecture.region?.name})
                            </option>
                        ))}
                    </select>
                </div>
                ...
                <div className="border-t pt-6 space-y-6">
                    ...
                    <div>
                        ...
                        <Textarea
                            ...
                            rows={4}
                            placeholder="Περιγράψτε πώς μπορεί κάποιος να φτάσει στην πόλη..."
                        />
                    </div>
